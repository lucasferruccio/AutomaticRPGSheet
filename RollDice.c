...
void roll_dice(int dice, int quantity);
...
void result(int modifier, int quantity, int value);

void calc_advdis(char dice[]);

void dice_main(char argv[])
{
    memset(result_dice, 0, 50);
    mod = 0;
    adv_dis = 0;
...
int checkdice(char dice[])
{
    int aux1, aux2 = 0, aux3, lim2 = strlen(dice);
